...
![로컬 우선 소형 모델 라우팅 구조를 표현한 hero image](assets/local-first-small-models/hero.png)
...
![local-first archive는 에이전트 세션을 재사용 가능한 지식으로 만든다](assets/local-first-small-models/local-archive.svg)
...
![민감 데이터는 로컬 필터와 정규화를 거친 뒤 큰 모델로 보낸다](assets/local-first-small-models/edge-pipeline.svg)
...
![작은 모델과 큰 모델의 역할 분담 라우팅](assets/local-first-small-models/task-routing.svg)
...
역할을 나누면 작은 모델을 어디에 둘지 더 선명해진다.

| 작업 | 작은 모델/로컬 처리 적합성 | 큰 모델 필요성 | 이유 |
|---|---|---|---|
| 문서 유형 분류 | 높음 | 낮음 | 반복적이고 label이 안정적임 |
| 개인정보 마스킹 | 높음 | 낮음~중간 | 원문을 외부로 보내기 전 처리 가능 |
| 주소·상호명 정규화 | 높음 | 낮음 | 규칙과 작은 모델 조합이 강함 |
| 세션 로그 검색 | 높음 | 중간 | 로컬 index와 요약 모델 조합이 효율적 |
| 복잡한 기획·판단 | 낮음 | 높음 | 긴 맥락과 추론이 필요함 |
| 최종 고객 답변 | 중간 | 높음 | 품질, 톤, 책임성이 중요함 |

핵심은 “작은 모델로 큰 모델을 대체하자”가 아니다. 큰 모델에게 가기 전에 입력을 정리하고, 위험한 데이터를 줄이고, 반복 작업을 가까운 곳에서 끝내자는 것이다.
...
문서 분류, 민감정보 마스킹, 로그 라벨링, 정규화, 세션 검색처럼 반복적이고 평가 기준이 비교적 명확한 작업에 적합하다. 복잡한 판단이나 긴 설명은 큰 모델에게 남기는 편이 안전하다.

### local-first AI의 장점은 무엇인가?

데이터를 가까운 곳에서 먼저 처리하므로 지연과 비용을 줄이고, 민감한 원문을 외부 API로 보내기 전에 필터링할 수 있다. 또한 세션 로그와 작업 흔적을 로컬에 쌓아 다음 작업의 컨텍스트로 재사용할 수 있다.
